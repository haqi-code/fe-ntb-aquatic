...
export default function AtletAdminIndependen() {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get("/independen-admin/atlet");
  ...
  const fetchById = async (id) => {
    try {
      const res = await api.get(`/independen-admin/atlet/${id}`);
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      if (editId) {
        await api.put(`/independen-admin/atlet/${editId}`, formData);
        setMessage("Data berhasil diupdate");
      } else {
        await api.post("/independen-admin/atlet", formData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/independen-admin/atlet/${id}`);
