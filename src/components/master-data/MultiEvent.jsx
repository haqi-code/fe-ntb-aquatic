...
export default function MultiEvent({setShowSidebarEvent} ) {
  const fields = [
    {
      id: "nama_event",
      label: "Nama Event",
  ...
  const fetchData = async () => {
    try {
      const res = await api.get("/admin/multi-event");
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      if (editId) {
        await api.put(`/admin/multi-event/${editId}`, formData, {
  ...
  const fetchById = async (id) => {
    try {
      const res = await api.get(`/admin/multi-event/${id}`);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/admin/multi-event/${id}`);
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between mb-4">
        <h1 className="font-bold text-xl">Master Data Multi Event</h1>
        <Button
            ...
            events.map((event) => (
              ...
              >
                ...
                <div className="flex justify-between items-center mt-auto">
                  ...
                  <Button text="Lihat Detail" onClick={() => setShowSidebarEvent(true)}/>
                </div>
              </div>
            ))}
        </div>
